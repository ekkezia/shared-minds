...
export default function DualTimeline({
  ...
  const timelineData = useMemo(() => {
    if (!callStartTime) return { mySegments: [], otherSegments: [] };
    ...
    const toRelativeTime = (timestamp) => {
      ...
      return (time - startTime) / 1000; // seconds from call start
    };
    ...
    sortedMyHistory.forEach((event) => {
      const eventTime = toRelativeTime(event.timestamp);
    ...
    const myLastChunkTime = myChunks.length > 0 && myChunks[myChunks.length - 1].created_at
      ? toRelativeTime(myChunks[myChunks.length - 1].created_at) + 5 // Add 5s for chunk duration
      : myLastTime + 10;
    ...
    if (otherChunks.length > 0) {
      ...
      otherChunks.forEach((chunk, idx) => {
        const chunkTime = chunk.created_at ? toRelativeTime(chunk.created_at) : otherLastTime;
      ...
      const totalDuration = Math.max(
        ...otherChunks.map((c) => 
          c.created_at ? toRelativeTime(c.created_at) + 5 : 0
        )
      );
    ...
    return { mySegments, otherSegments };
  }, [chunks, callStartTime, myStateHistory, otherStateHistory, myNorm]);

  // Calculate total duration
  const totalDuration = useMemo(() => {
    if (!callStartTime) return 60; // Default 60 seconds
